...
class SpacetimeDBClient {
    ...
    static getReducerClass(name) {
        ...
        if (!reducerClass) {
            console.warn(`Could not find class \"${name}\", you need to register it with SpacetimeDBClient.registerReducer() first`);
            return;
        }
        return reducerClass;
    }
    ...
    processMessage(wsMessage, callback) {
        if (this.protocol === "binary") {
            ...
            data.arrayBuffer().then((data) => {
                const message = Proto.Message.decode(new Uint8Array(data));
                if (message["subscriptionUpdate"]) {
                    ...
                    const reducerName = toPascalCase(originalReducerName);
                    const args = functionCall.argBytes;
                    const status = Proto.event_StatusToJSON(event.status);
                    const messageStr = event.message;
                    const transactionUpdateEvent = new TransactionUpdateEvent(identity, address, originalReducerName, reducerName, args, status, messageStr);
                    const transactionUpdate = new TransactionUpdateMessage(subscriptionUpdate.tableUpdates, transactionUpdateEvent);
                    ...
                    const identityTokenMessage = new IdentityTokenMessage(identity, token, address);
            ...
            if (data["SubscriptionUpdate"]) {
                ...
                const transactionUpdateEvent = new TransactionUpdateEvent(identity, address, originalReducerName, reducerName, args, status, message);
                const transactionUpdate = new TransactionUpdateMessage(subscriptionUpdate.tableUpdates, transactionUpdateEvent);
                callback(transactionUpdate);
            }
